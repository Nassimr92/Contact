...
export default {
  ...
  computed: {
    ...
    Friendsname() {
      ...
      return person ? person.nom : 'Contact non trouvé';
    }
  },
  methods: {
    addNumber(number) {
    ...
    call() {
      ...
      const nom = this.Friendsname !== 'Contact non trouvé' ? this.Friendsname : 'Inconnu';
      this.journalStore.appel({ nom: nom, date: date });
...
<style scoped>
  .clavier-global-conteneur{
    border: 3px solid white(245, 155, 20);
    padding: 20px;
    display: grid;
